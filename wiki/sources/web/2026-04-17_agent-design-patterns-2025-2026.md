...
used_for: src/content/ai-agents-advanced.html Ch.04 · wiki/entities/{react,reflection,plan-and-execute,orchestrator-worker,mcp}.md
---
...
본 사이트의 Part 4 Ch.04 "에이전트 디자인 패턴 5선 — 2026년 표준"을 2025-2026 시점 사실 기반으로 보강하기 위한 리서치. 기존 챕터는 패턴별 개요만 있고, **2025년의 Pre-Act/MAR/Deep Agents/MCP 표준화/Anthropic 5패턴 분류 정착** 등 *최근 1년의 결정적 변화*를 반영하지 못했음. 본 소스에 모든 정량 데이터·인용·논문·블로그 URL을 보존해 추후 Lint·재사용 근거가 됨.
...
- 다이어그램 참조: Task → Agent[LLM ↔ Tools] → Environment with Action/Result loop, LLM has Reasoning self-loop. 본 사이트 Part 4 Ch.04 ReAct 아키텍처 SVG의 직접 참조본.
...
- src/content/ai-agents-advanced.html — Part 4 Ch.04 "에이전트 디자인 패턴 5선" 전면 보강

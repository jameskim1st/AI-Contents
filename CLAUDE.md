...
**메타:** 이 파일 자체가 Part 5 Ch.05 "Harness Engineering"과 Ch.09 "LLM Wiki"에서
가르치는 패턴의 살아있는 적용입니다.
...
| 2 | `/02-ai-agents` | teal | AI Agents 기초 |
| 3 | `/03-ai-agents-advanced` | violet | AI Agents 실전 |
| 4 | `/04-vibe-basic` | accent (orange) | 바이브코딩 입문 |
| 5 | `/05-vibe-master` | accent (orange) | 바이브코딩 심화 |
| 6 | `/06-ai-dev` | accent (orange) | AI 개발 |
| 7 | `/07-data-basics` | amber | Data 기초 |
| 8 | `/08-data-enterprise` | amber | Data 실전 |
| 9 | `/09-it-basics` | green | 개발 배경지식 — IT 기초 |
| 10 | `/10-power-platform` | blue | Power Platform |
...
- Part 2 (AI Agents 기초): `var(--teal)` / `--teal-bg` / `--teal-border`
- Part 3 (AI Agents 실전): `var(--violet)` 계열
- Part 4·5·6 (Vibe·AI개발): `var(--accent)` / `--accent-bg` / `--accent-border`
- Part 7·8 (Data): `var(--amber)` / `--amber-bg` / `--amber-border`
- Part 9 (IT 기초): `var(--green)` / `--green-bg` / `--green-border`
- Part 10 (Power Platform): `var(--blue)` / `--blue-bg` / `--blue-border`
...
- **링크는 Vercel 배포 URL** 사용:
  ```
  ✓ https://ai-contents-wine.vercel.app/07-data-basics/#anchor
...
- 빌드 실패하면 절대 푸시하지 말 것
- 12페이지 모두 정상 생성 확인 (Overview + 11 parts)
...
자세히는 `wiki/schema.md`와 [Part 5 Ch.09 LLM Wiki](https://ai-contents-wine.vercel.app/05-vibe-master/#vm-ch9).
...
- **본 사이트가 LLM Wiki를 직접 운영하는 살아있는 데모:** [Part 5 Ch.09](https://ai-contents-wine.vercel.app/05-vibe-master/#vm-ch9)
